...
function getWeather(callback) {
	...
	d3.csv(filename)
		.row(function(readings){
			var timestamp = readings.Date + " " + readings.TimeEDT;
			timestamp = moment(timestamp, "MM/DD/YY hh:mm A");
			
		  var slice = readings.Precip.split(/\s/);
			
			return {
				y: +slice[0],
				x: moment.utc(timestamp).valueOf()
			};
		})
	.get(function(error, rows){
		second_y = rows;
